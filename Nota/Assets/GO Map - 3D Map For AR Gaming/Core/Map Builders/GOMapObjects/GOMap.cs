...
namespace GoMap
{
	...
	public class GOMap : MonoBehaviour 
	{
		...
		void Start () 
	    {
			FirebaseDatabase.DefaultInstance.GetReference("world").ValueChanged += (object sender2, ValueChangedEventArgs e2) =>
			 {
...
//			goTile.verticesInWorld = goTile.vertsInWorld (zoomLevel);
		...
		public void dropPin(Coordinates coordinates, GameObject go) {
			...
			Instantiate(go).transform.localPosition = coordinates.convertCoordinateToVector(0);
